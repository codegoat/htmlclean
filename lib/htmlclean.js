...
module.exports = function(html, options) {
  ...
  html = html.replace(/(<[^\S\f]*(textarea|script|style|pre)\b[^>]*>)([\s\S]*?)(<[^\S\f]*\/[^\S\f]*\2\b[^>]*>)/ig,
    function(str, startTag, tagName, innerHtml, endTag) {
      ...
      if (innerHtml !== '') {
        ...
        } else if (tagName.toLowerCase() === 'script' && options && options.processScriptContentType) {
          for (var h = 0; h < options.processScriptContentType.length; h++) {
            if (startTag.match(new RegExp('type=[\'"]' + options.processScriptContentType[h] + '[\'"]'))) {
  ...
  html = html.replace(/[\n\r\t]+/g, '').replace(/[ ]{2,}/g, ' '); // \s includes many others.
